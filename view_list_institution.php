...
$page_title = 'View List of Institutions';
    ...
    <section class="mb-4">
      <div>
        ...
        echo ' <div class="container mx-auto" style="width: 75rem;"><br>
            <div class="card text-center bg-light text-dark bg-opacity-100" style="width: 75rem;">
              <div class="card-header">
                <img src="images/i.png" class="p-2 my-2" height="200" alt="Institution Logo" loading="lazy" style="margin-top: -1px />
              </div>
          <div class=" card-body text-start">';

        //Search form
        $search_query = isset($_GET['search_query']) ? $_GET['search_query'] : '';
        echo '<form class="m-3 d-flex justify-content-center" method="GET" action="view_list_institution.php">
        ...
        if (isset($_GET["search_query"])) {
          ...
          $sql = "SELECT * FROM institution WHERE name LIKE '%$search_query%' OR email LIKE '%$search_query%'";
          ...
          if ($num > 0) {     //If it is found..
            ...
            echo "<p><b>$num</b> institutions found.</p>\n</center>";
            ...
            while ($row = $r->fetch_assoc()) {
              echo '<tr>
                  ...
                  <td align="center">
                    <div>
                      <a style="background-color: #89C4E1;" class="btn btn-secondary" role="button" href="view_details_institution.php?id=' . $row['id'] . '"><i class="far fa-eye"></i></a>
                    </div>
                  </td>
                  <td align="center">
                    <div>
                      <a style="background-color: #518cbb;" class="btn btn-danger" role="button" href="delete_institution.php?id=' . $row['id'] . '"><i class="fas fa-trash"></i></a>
                    </div>
                  </td>
                </tr>';
            }
          ...
          $sql = "SELECT institution.id, institution.name, institution.email, institution.contact_number FROM institution LIMIT $offset, $records_per_page";
          $result = mysqli_query($dbc, $sql);
        ...
        <lottie-player src="https://assets10.lottiefiles.com/packages/lf20_zyjeeirq.json"  background="transparent"  speed="1" class="mb-2" style="width: 150px; height: 150px; margin-top: -5px;"  loop  autoplay></lottie-player>';
            echo "<p>There are currently <b>{$row['total']}</b> institution registered.</p></center>";
            ...
            if ($current_page > 1) {
              echo "<a href='view_list_institution.php?page=" . ($current_page - 1) . "'>Previous &nbsp</a> ";
            }
            if ($start_range > 1) {
              echo "<a href='view_list_institution.php?page=1'>1 &nbsp</a> ";
              if ($start_range > 2) {
                echo "... ";
              }
            }
            for ($i = $start_range; $i <= $end_range; $i++) {
              if ($i == $current_page) {
                ...
                echo "<a href='view_list_institution.php?page=$i'>$i &nbsp</a> ";
              }
            }
            if ($end_range < $total_pages) {
              ...
              echo "<a href='view_list_institution.php?page=$total_pages'>$total_pages &nbsp</a> ";
            }
            if ($current_page < $total_pages) {
              echo "<a href='view_list_institution.php?page=" . ($current_page + 1) . "'>Next &nbsp</a> ";
            }
            echo "</div></div>";
            ...
            echo '<center><script src="https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"></script>
              <lottie-player src="https://assets1.lottiefiles.com/packages/lf20_s0fkqiqh.json"  background="transparent"  speed="1"  style="width: 200px; height: 200px; margin-top:-15px"  loop  autoplay></lottie-player>';
            echo '<p class="error">There are currently no institution.</p></center>';
          }
        }
