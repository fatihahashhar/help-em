...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	// Find id from table institution in database
	$q = "SELECT * FROM institution WHERE email='$email' AND password=SHA1('$password')";
	...
	if (mysqli_num_rows($r) == 1) {	// If found
		...
		if ($result_fetch['is_verified'] == 1) {
			$_SESSION["institution_id"] = $result_fetch['id'];
			$_SESSION["username"] = $result_fetch['username'];
			echo '
			<script>
				alert("Login successful");
				window.location.href = "view_voluntary_work_list_institution.php";
			</script>';
...
?>
	<form class="px-2" action="login_institution.php" method="post">
		<fieldset>
	...
	<div class="card-footer text-muted">Don't have an account yet? <br> <a href="register_institution.php">Create New Account</a></div>
	</div>
	</div>
	</div>
	</div>
