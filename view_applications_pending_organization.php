...
$page_title = 'View Pending Applications';
            ...
            <div class="container mx-auto" style="width: 75rem;">
              <div class="card text-center bg-light text-dark bg-opacity-100" style="width: 75rem;">
                  <div class="card-header">
                    <img src="images/pa.png" class="p-2 my-3" height="200" alt="Application Logo" loading="lazy" style="margin-top: -1px />
                  </div>
                  <div class="card-body text-start">';
                    ...
                    (
                      SELECT id FROM voluntary_work_organization WHERE organization_id = $org_id AND application.status != 'REJECTED' AND application.status != 'APPROVED' AND application.status != 'PARTICIPATE' 
                      AND application.status != 'COMPLETED' AND application.status != 'ABSENT' AND application.status != 'DONE'
                    )";
                    ...
                    echo '
                    <form id="search_box" class="m-3 d-flex justify-content-center" method="GET" action="view_applications_pending_organization.php">
                    ...
                    if (isset($_GET["search_query"])) {
                      ...
                      $sql = "SELECT id, status, volunteer_id, voluntary_work_id, voluntary_work_name, volunteer_username, volunteer_contact_number, organizer_of_voluntary_work_id
                              FROM
                              (
                                  SELECT application.id, application.status, application.volunteer_id, voluntary_work_organization.id AS voluntary_work_id, 
                                    ...
                                    voluntary_work_organization.organization_id AS organizer_of_voluntary_work_id 		
                                  FROM application
                                  ...
                                  WHERE voluntary_work_organization.organization_id = $org_id AND application.status != 'REJECTED' AND application.status != 'APPROVED' AND application.status != 'PARTICIPATE' 
                                  AND application.status != 'COMPLETED' AND application.status != 'ABSENT' AND application.status != 'DONE'
                                  ...
                                  voluntary_work_institution.institution_id AS organizer_of_voluntary_work_id
                                  FROM application
                                  LEFT JOIN voluntary_work_institution ON application.voluntary_work_institution_id = voluntary_work_institution.id
                                  LEFT JOIN voluntary_work_organization ON application.voluntary_work_organization_id = voluntary_work_organization.id
                                  ...
                                  WHERE voluntary_work_name IS NOT NULL AND organizer_of_voluntary_work_id = $org_id AND status != 'APPROVED' AND status !='REJECTED' AND status != 'PARTICIPATE' 
                                  AND status != 'COMPLETED' AND status != 'ABSENT' AND status != 'DONE'";
                            ...
                            <tr>
                              ...
                              <td align="center">
                                ...
                                if ($status != 'REJECTED') { // If the status is 'PENDING'
                                  echo '<a style="background-color: #89C4E1;" class="btn btn-secondary" role="button" href="view_applicants_details_pending_organization.php?id=' . $row['volunteer_id'] . '">VIEW</a>';
                              ...
                              <td align="center">
                                <div>';
                                  // Check if status is 'APPROVED' or 'REJECTED' to disable the button
                                  if ($status == 'PENDING') { // If the status is 'PENDING'
                                    echo '<a style="background-color: #54B435;" class="btn btn-secondary" role="button" href="approve_volunteer_application_organization.php?id=' . $row['id'] . '"><i class="fa-solid fa-square-check"></i></a>';
                                  } else { // If the status is 'APPROVED' or 'REJECTED'
                              ...
                              <td align="center">
                                <div>';
                                  // Check if status is 'APPROVED' or 'REJECTED' to disable the button
                                  if ($status == 'PENDING') { // If the status is 'PENDING'
                                    echo '<a style="background-color: #DF2E38;" class="btn btn-danger" role="button" href="reject_volunteer_application_organization.php?id=' . $row['id'] . '"><i class="fas fa-trash"></i></a>';
                                  } else { // If the status is 'APPROVED' or 'REJECTED'
                    ...
                    <div class="mb-2">';
                      ...
                      if ($current_page > 1) {
                        echo "<a href='view_applications_pending_organization.php?page=" . ($current_page - 1) . "'>Previous &nbsp</a> ";
                      }
                      if ($start_range > 1) {
                        echo "<a href='view_applications_pending_organization.php?page=1'>1 &nbsp</a> ";
                        if ($start_range > 2) {
                          echo "... ";
                        }
                      }
                      for ($i = $start_range; $i <= $end_range; $i++) {
                        if ($i == $current_page) {
                          ...
                          echo "<a href='view_applications_pending_organization.php?page=$i'>$i &nbsp</a> ";
                        }
                      }
                      if ($end_range < $total_pages) {
                        ...
                        echo "<a href='view_applications_pending_organization.php?page=$total_pages'>$total_pages &nbsp</a> ";
                      }
                      if ($current_page < $total_pages) {
                        echo "<a href='view_applications_pending_organization.php?page=" . ($current_page + 1) . "'>Next &nbsp</a> ";
                      };
                      ...
                      if (mysqli_num_rows($result) > 0) {
                        ...
                        echo "<center><p>There are currently <b>{$row['total']}</b> pending applications</p><br></center>";
                  ...
                  if ($current_page < $total_pages) {
                    ...
                    mysqli_free_result($result); // Free memory associated with $r	
                        ...
                        echo '<p class="error">There are currently no application that are pending</p><br></center>';
                      }
