		...
		<div class="container mx-auto" style="width: 75rem;">
            <div class="card px-3 text-center bg-light text-dark bg-opacity-100" style="width: 75rem;">
          	...
          	<div class=" card-body text-start">';
				...
				echo '
				<form id="search_box" class="m-3 d-flex justify-content-center" method="GET" action="view_voluntary_work_list_institution.php">
				...
				if (isset($_GET["search_query"])) {
					...
					$sql = "SELECT * FROM voluntary_work_institution
							WHERE (name LIKE '%$search_query%' OR task LIKE '%$search_query%') 
							AND institution_id = $inst_id AND status != 'FINISHED'";
					...
					if ($num > 0) {     //If it is found..
						...
						while ($row = $r->fetch_assoc()) {
							...
							<td align="center">
								<div>
									<a style="background-color: #89C4E1;" class="btn btn-secondary" role="button" href="edit_details_voluntary_work_institution.php?id=' . $row['id'] . '"><i class="fas fa-edit"></i></a>
								</div>
							</td>
							<td align="center">
								<div>
									<a style="background-color: #518cbb;" class="btn btn-danger" role="button" href="delete_voluntary_work_institution.php?id=' . $row['id'] . '"><i class="fas fa-trash"></i></a>
								</div>
							</td>
                		</tr>';
						}
						...
						echo' 
							...
							if ($current_page > 1) {
								echo "<a href='view_voluntary_work_list_institution.php?page=" . ($current_page - 1) . "'>Previous &nbsp</a> ";
							}
							if ($start_range > 1) {
								echo "<a href='view_voluntary_work_list_institution.php?page=1'>1 &nbsp</a> ";
								if ($start_range > 2) {
									echo "... ";
								}
							}
							for ($i = $start_range; $i <= $end_range; $i++) {
								if ($i == $current_page) {
									...
									echo "<a href='view_voluntary_work_list_institution.php?page=$i'>$i &nbsp</a> ";
								}
							}
							if ($end_range < $total_pages) {
								...
								echo "<a href='view_voluntary_work_list_institution.php?page=$total_pages'>$total_pages &nbsp</a> ";
							}
							if ($current_page < $total_pages) {
								echo "<a href='view_voluntary_work_list_institution.php?page=" . ($current_page + 1) . "'>Next &nbsp</a> ";
							}
					...
					FROM voluntary_work_institution WHERE institution_id = $inst_id AND voluntary_work_institution.status != 'FINISHED' ORDER BY voluntary_work_institution.date LIMIT $offset, $records_per_page";
					$result = mysqli_query($dbc, $sql);
