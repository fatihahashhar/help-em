...
function sendMail($em, $v_code) {
	...
	$mail->Body = "Thank you for your registration !
		...
		<a href='http://localhost/Help_em/verify_institution.php?email=$em&v_code=$v_code'>Verify</a>";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	// If everything's OK. ($na && $em && $ad && $pc && $ct && $st && $cn && && $bn && $an && $pw)
	if (empty($errors)) {

		// Register the reservation in the database...
		require('../mysqli_connect_fyp.php'); // Connect to the db.

		$user_exist_query = "SELECT * FROM institution WHERE email='$em'";
		...
		if ($s) { 
			if(mysqli_num_rows($s)>0) {
				...
				$q = "INSERT INTO institution (name, email, address, postcode, city, state, contact_number, bank_name, account_number, password, verification_code, is_verified) VALUES ('$na', '$em', '$ad', '$pc', '$ct', '$st', '$cn', '$bn', '$an', SHA1('$pw'), '$v_code', '0')";
				
				if (mysqli_query($dbc, $q) && sendMail($_POST['email'], $v_code)){
					$u = "SELECT id FROM institution WHERE email='$em' and password=SHA1('$pw')";
...
<div class="card-body text-start">
	<form class="row g-3 px-3 needs-validation" action="register_institution.php" method="post">
	<fieldset>';
	echo '
	<div class="form-group row mb-3">
	<div class="col-sm-6">
	<div class="input-group mb-1">
	<span class="input-group-text" id="basic-addon1">Institution Name</span>
  ...
  <div class="col-sm-6">
  <div class="input-group mb-1">
  <span class="input-group-text" id="basic-addon1">Institution Email</span>
...
<span class="input-group-text" id="basic-addon1">Address</span>
  ...
  <span class="input-group-text" id="basic-addon1">Postcode</span>
    <input required type="text" onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))" class="form-control" id="inputPostcode" name="postcode" size="5" maxlength="5" value="';
...
?>
	...
	<div align="right"><input class="btn btn-primary btn-lg col-sm-3 mb-2" type="submit" name="submit" value=" Submit " /></div>
	</form>
	</div>
	<div class="card-footer text-muted">Already have an account? <a href="login_institution.php">Login Now!!!</a></div>
	</div>
	</div>
	</div>
	</div>
