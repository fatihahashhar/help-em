              ...
              <div class="card text-center bg-light text-dark bg-opacity-100" style="width: 75rem;">
                  ...
                  <div class="card-body text-start">';
                    ...
                    $sql = "SELECT COUNT(*) AS total FROM application WHERE voluntary_work_organization_id IN 
                              (
                                SELECT id FROM voluntary_work_organization WHERE organization_id = $org_id AND application.status != 'REJECTED' AND application.status != 'PENDING' 
                    ...
                    echo '
                    <form id="search_box" class="m-3 d-flex justify-content-center" method="GET" action="view_applications_approved_organization.php">
                    ...
                    if (isset($_GET["search_query"])) {
                      ...
                      $sql = "SELECT id, status, volunteer_id, voluntary_work_id, voluntary_work_name, volunteer_username, volunteer_contact_number, organizer_of_voluntary_work_id, volunteer_email
                              ...
                              (
                                  ...
                                  WHERE voluntary_work_organization.organization_id = $org_id AND application.status != 'REJECTED' AND application.status != 'PENDING' 
                                  AND application.status != 'ABSENT' AND application.status != 'PARTICIPATE' AND application.status != 'COMPLETED' AND application.status != 'DONE'
                                  ...
                                  WHERE voluntary_work_name IS NOT NULL AND organizer_of_voluntary_work_id = $org_id AND status != 'PENDING' AND status !='REJECTED' 
                  ...
                  echo' 
                    <div class="mb-2">';
                      ...
                      if ($current_page > 1) {
                        echo "<a href='view_applications_approved_organization.php?page=" . ($current_page - 1) . "'>Previous &nbsp</a> ";
                      }
                      if ($start_range > 1) {
                        echo "<a href='view_applications_approved_organization.php?page=1'>1 &nbsp</a> ";
                        if ($start_range > 2) {
                          echo "... ";
                        }
                      }
                      for ($i = $start_range; $i <= $end_range; $i++) {
                        if ($i == $current_page) {
                          ...
                          echo "<a href='view_applications_approved_organization.php?page=$i'>$i &nbsp</a> ";
                        }
                      }
                      if ($end_range < $total_pages) {
                        ...
                        echo "<a href='view_applications_approved_organization.php?page=$total_pages'>$total_pages &nbsp</a> ";
                      }
                      if ($current_page < $total_pages) {
                        echo "<a href='view_applications_approved_organization.php?page=" . ($current_page + 1) . "'>Next &nbsp</a> ";
                      };
                    ...
                    (
                        ...
                        LEFT JOIN organization ON voluntary_work_organization.organization_id = organization.id
                        LEFT JOIN volunteer ON application.volunteer_id = volunteer.id
                        WHERE voluntary_work_organization.organization_id = $org_id AND application.status != 'REJECTED' AND application.status != 'PENDING' AND application.status != 'PARTICIPATE' 
                              ...
                              echo '
                                <tr>
                                  ...
                                  <td align="center">
                                    ...
                                    // Check if status is 'APPROVED' or 'REJECTED' to disable the button
                                    if ($status != 'REJECTED') { // If the status is 'PENDING'
                                      echo '<a style="background-color: #89C4E1;" class="btn btn-secondary" role="button" href="view_applicants_details_approved_organization.php?id=' . $row['volunteer_id'] . '">VIEW</a>';
                                    } else { // If the status is 'APPROVED' or 'REJECTED'
                                      echo '<button class="btn btn-danger" style="background-color: #89C4E1;" disabled>VIEW</button>';
                                    }
                                    echo'</div>
