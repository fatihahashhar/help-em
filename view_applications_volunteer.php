...
$page_title = 'View List of Applications';
    ...
    <div class="py-4 col-xl-15" style="padding-left: 12px;">
      <div class="card text-center bg-light text-dark bg-opacity-100 w-100">
        <div class="card-header">
            <img src="images/a.png" class="p-3 mb-2" height="200" alt="Application Logo" loading="lazy" style="margin-top: -1px />
        </div>
        <div class="card-body text-start">';
          ...
          $sql = "SELECT COUNT(*) AS total FROM application WHERE volunteer_id = $vol_id AND status !='ABSENT' AND status !='PARTICIPATE' AND status !='COMPLETED' AND status !='DONE'";
          ...
          echo '
          <form id="search_box" class="m-3 d-flex justify-content-center" method="GET" action="view_applications_volunteer.php">
          ...
          if (isset($_GET["search_query"])) {
            ...
            $sql = "SELECT id, app_status AS status, volunteer_id, organizer_name, voluntary_work_name, voluntary_work_date, organizer_contact_number, voluntary_work_id
                    FROM (
                        ...
                        WHERE volunteer_id = $vol_id AND application.status != 'DONE' AND application.status != 'ABSENT' AND application.status != 'PARTICIPATE' AND application.status != 'COMPLETED'
                    ...
                    WHERE organizer_name IS NOT NULL AND volunteer_id = $vol_id AND app_status != 'ABSENT' AND app_status != 'PARTICIPATE' AND app_status != 'COMPLETED' AND app_status != 'DONE'";
                        ...
                        echo '
                          <tr>
                            ...
                            <td align="center">
                              ...
                              // Check if status is other than 'APPROVED' to disable the button
                              if ($status != 'APPROVED') {
                                echo '<a class="btn btn-danger" role="button" href="remove_application_volunteer.php?id=' . $row['id'] . '"><i class="fas fa-trash"></i></a>';
                              } else {
            ...
            <div class="mb-2">';
              ...
              if ($current_page > 1) {
                echo "<a href='view_applications_volunteer.php?page=" . ($current_page - 1) . "'>Previous &nbsp</a> ";
              }
              if ($start_range > 1) {
                echo "<a href='view_applications_volunteer.php?page=1'>1 &nbsp</a> ";
                if ($start_range > 2) {
                  echo "... ";
                }
              }
              for ($i = $start_range; $i <= $end_range; $i++) {
                if ($i == $current_page) {
                  ...
                  echo "<a href='view_applications_volunteer.php?page=$i'>$i &nbsp</a> ";
                }
              }
              if ($end_range < $total_pages) {
                ...
                echo "<a href='view_applications_volunteer.php?page=$total_pages'>$total_pages &nbsp</a> ";
              }
              if ($current_page < $total_pages) {
                echo "<a href='view_applications_volunteer.php?page=" . ($current_page + 1) . "'>Next &nbsp</a> ";
              };
              ...
              if (mysqli_num_rows($result) > 0) {
                ...
                echo "<center><p>There are currently <b>{$row['total']}</b> application sent.</p><br></center>";
            ...
            mysqli_free_result($result); // Free memory associated with $r	
                ...
                echo '<p class="error">There are currently no application sent.</p></center>';
              }
