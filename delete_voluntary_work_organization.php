...
// This page is accessed through view_voluntary_work_list_organization.php.
...
<div class="card-body text-start">';
				if ((isset($_GET['id'])) && (is_numeric($_GET['id']))) { // From view_voluntary_work_list_organization.php
					$id = $_GET['id'];
...
<lottie-player src="https://assets6.lottiefiles.com/packages/lf20_j3UXNf.json"  background="transparent"  speed="1"  style="width: 150px; height: 150px;"  loop  autoplay></lottie-player></br>';
				...
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
					if ($_POST['sure'] == 'Yes') { // Delete the record.
						...
						// Make the query:
						$q = "DELETE FROM voluntary_work_organization WHERE voluntary_work_organization.id = $id";
...
<p class="error">The voluntary work could not be deleted due to a system error.</p>'; // Public message.
					...
					$q = "SELECT voluntary_work_organization.name FROM voluntary_work_organization WHERE voluntary_work_organization.id = $id";
...
Are you sure you want to <b>DELETE</b> this voluntary work from the system? ";
						...
						// Create the form:
						echo '<form action="delete_voluntary_work_organization.php" method="post">
<br/>
<input type="radio" name="sure" value="Yes" /> <b>Yes</b> &nbsp &nbsp
<input type="radio" name="sure" value="No" checked="checked" /> <b>No</b> <br/><br/>
<div align="center"><input style="background-color: #518cbb;" class="btn btn-primary btn-lg" type="submit" name="submit" value="Confirm" /> &nbsp&nbsp
<a href="view_voluntary_work_list_organization.php" style="background-color: #89C4E1;" class="btn btn-secondary btn-lg" tabindex="-1" role="button" aria-disabled="true">CANCEL</a></div>
