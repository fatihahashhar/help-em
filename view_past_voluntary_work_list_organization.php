		...
		<div class="container mx-auto" style="width: 75rem;">
            <div class="card text-center bg-light text-dark bg-opacity-100" style="width: 75rem;">
          	...
          	<div class=" card-body text-start">';
				...
				echo '<form class="m-3 d-flex justify-content-center" method="GET" action="view_past_voluntary_work_list_organization.php">
				...
				if (isset($_GET["search_query"])) {
					...
					$sql = "SELECT * FROM voluntary_work_organization WHERE (name LIKE '%$search_query%' OR task LIKE '%$search_query%' OR date LIKE '%$search_query%') 
          AND organization_id = $org_id AND voluntary_work_organization.status = 'FINISHED'";
					...
					if ($num > 0) {     //If it is found..
						...
						while ($row = $r->fetch_assoc()) {
							...
							<td align="center">
								<div>
									<a style="background-color: #89C4E1;" class="btn btn-secondary" role="button" href="view_past_voluntary_work_details_organization.php?id=' . $row['id'] . '">VIEW</a>
								</div>
							</td>
                			</tr>';
						}
						...
						if ($current_page > 1) {
							echo "<a href='view_past_voluntary_work_list_organization.php?page=" . ($current_page - 1) . "'>Previous &nbsp</a> ";
						}
						if ($start_range > 1) {
							echo "<a href='view_past_voluntary_work_list_organization.php?page=1'>1 &nbsp</a> ";
							if ($start_range > 2) {
								echo "... ";
							}
						}
						for ($i = $start_range; $i <= $end_range; $i++) {
							if ($i == $current_page) {
								...
								echo "<a href='view_past_voluntary_work_list_organization.php?page=$i'>$i &nbsp</a> ";
							}
						}
						if ($end_range < $total_pages) {
							...
							echo "<a href='view_past_voluntary_work_list_organization.php?page=$total_pages'>$total_pages &nbsp</a> ";
						}
						if ($current_page < $total_pages) {
							echo "<a href='view_past_voluntary_work_list_organization.php?page=" . ($current_page + 1) . "'>Next &nbsp</a> ";
						}
					...
					$sql = "SELECT voluntary_work_organization.id, voluntary_work_organization.name, voluntary_work_organization.task, voluntary_work_organization.date, voluntary_work_organization.status 
          				FROM voluntary_work_organization WHERE organization_id = $org_id AND voluntary_work_organization.status = 'FINISHED' LIMIT $offset, $records_per_page";
					$result = mysqli_query($dbc, $sql);
