...
<div class="container-fluid">
	...
	echo'
	<div class="alert alert-warning alert-dismissible fade" role="alert" id="warningAlert" style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 9999; display: none;">
		<strong>Sorry.</strong> <br> 
		This voluntary work is not yet open for registration. <br>
		Please check back later for updates. <i class="fa-solid fa-face-smile-wink"></i>
		<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close" onclick="hideWarningAlert()"></button>
	</div>';
							...
							<div class="card-body text-start">
								<div class="card-body">
									<form class="row g-3 px-2" action="read_more_voluntary_work_details_volunteer.php?id=' . $id . '" method="post">
										...
										<div align="right">
											<a href="javascript:history.back()" style="background-color: #89C4E1; margin-bottom: -6px; margin-left: 20px;" class="btn btn-danger btn-lg col-sm-2" tabindex="-1" role="button" aria-disabled="true">BACK</a>';
											if ($status == 'OPEN FOR REGISTRATION') {
												echo' <a style="background-color: #518cbb; margin-bottom: -6px; margin-left: 20px;" class="btn btn-secondary btn-lg col-sm-3" tabindex="-1" role="button" aria-disabled="true" href="apply_voluntary_work_volunteer.php?id=' . $row['id'] . '"> APPLY NOW !</a>';
											} else if ($status == 'FINISHED'){
											} else {
												echo' <a style="background-color: #B2B2B2; margin-bottom: -6px; margin-left: 20px;" class="btn btn-secondary btn-lg col-sm-3" tabindex="-1" role="button" onclick="showWarningAlert()">APPLY NOW !</a>';
